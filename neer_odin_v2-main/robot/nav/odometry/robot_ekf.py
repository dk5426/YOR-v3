...
class RobotEKF:
    ...
    def __init__(
        ...
        # ZED measurement noise std (these are the MINIMUM / best-case values;
        # the update() caller can pass R_override to inflate them on bad frames)
        r_xy: float = 0.0024,   # m  — nominal ZED position noise at full confidence
        r_th: float = 0.0006,   # rad — nominal ZED heading noise
    ):
        # State [px, pz, theta]
        self.x = np.zeros(3)
        # Covariance — start uncertain so first ZED fix dominates
        self.P = np.eye(3) * 0.1
    ...
    def predict(self, u: np.ndarray, dt: float) -> None:
        ...
        Kinematics (F6 — empirically verified against ZED ground truth):
        ...
        # Process model — F6 (empirically verified against ZED ground truth):
    ...
    def update(
        ...
        if self._update_calls == 1:
            print(f"[EKF.update] *** FIRST ZED UPDATE ACCEPTED ***")
    ...
    def update_orb(
        ...
        Args:
            ...
            gate_chi2:       Mahalanobis chi-squared threshold (3 DOF).
                             Wider than ZED gate (50 vs 12) to accept large loop-closure
