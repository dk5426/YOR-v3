...
class PicoLift:
    ...
    _VOLTAGE_PATTERN = re.compile(r"Voltage:\s*([\d.]+)\s*V")
    ...
    def __init__(
        ...
        self._voltage_v: Optional[float] = None
    ...
    def _drain_loop(self) -> None:
        ...
        while not self._drain_stop.is_set():
            try:
                ...
                # Auto-ping for battery/status every 1 second
                ...
                if data:
                    try:
                        ...
                        # Only print state transitions, suppress noisy MOVING lines
                        ...
                        v_match = self._VOLTAGE_PATTERN.search(line)
                        if v_match:
                            voltage = float(v_match.group(1))
                            
                            # Calculate battery percentage for 24V 7S Li-ion (29.4V full, 21.0V empty)
                            v_max = 29.4
                            v_min = 21.0
                            pct = max(0.0, min(100.0, (voltage - v_min) / (v_max - v_min) * 100.0))
...
class Base:
    ...
    def get_battery_voltage(self) -> Optional[float]:
        if self._pico_lift:
            return self._pico_lift.get_battery_voltage()
        return None
        
    def get_battery_percent(self) -> Optional[float]:
        if self._pico_lift:
            return self._pico_lift.get_battery_percent()
        return None
