...
def test_wrist_axis_tables():
    ...
    for side in SIDES:
        ...
        check(
            f"convention_matrix({side}) is a proper rotation",
            close(C @ C.T, np.eye(3), 1e-12) and abs(np.linalg.det(C) - 1.0) < 1e-12,
            f"det {np.linalg.det(C):+.6f}",
        )
        for axis in AXES:
            check(
                f"C maps {side} {axis} axis MANO→YOR",
                close(
                    C @ MANO_WRIST_AXES[side][axis], YOR_WRIST_AXES[side][axis], 1e-12
                ),
            )
...
def test_convention_matrix_matches_the_mjcf_hand_mount():
    ...
    for side in SIDES:
        ...
        check(
            f"{side} convention == MJCF arm_ee→wuji_hand_orient",
            err < 1e-3,
            f"max |Δ| {err:.1e}",
        )
...
def test_hand_joints():
    ...
    for side in SIDES:
        ...
        check(
            f"{side} hand joints match wuji-description order",
    ...
    for side in SIDES:
        ...
        check(
            f"{side} qpos write reaches the fingertips",
            close(d.qpos[adrs], hi) and moved > 0.01,
            f"tip moved {moved * 100:.1f} cm",
        )
...
def test_clutch_engage_release():
    ...
    clutch = Clutch("left")
    check(
        "inert until engaged", not clutch.engaged and clutch.target(np.eye(4)) is None
    )
    ...
    for side in SIDES:
        ...
        check(
            f"{side} target at engage is the engage pose",
...
def test_clutch_translation_wrist_frame():
    ...
    for side in SIDES:
        for axis in AXES:
            ...
            ee0 = mink.SE3.from_rotation_and_translation(
                mink.SO3.identity(), np.zeros(3)
            )
            c = Clutch(side, translation_frame="wrist")
            c.engage(np.eye(4), ee0)
            wrist = _pose(np.eye(3), 0.10 * MANO_WRIST_AXES[side][axis])
            check(
                f"{side} push along {axis} lands on the robot's {axis}",
                close(c.target(wrist).translation(), 0.10 * YOR_WRIST_AXES[side][axis]),
            )
    ...
    for engage_yaw in (0.0, 0.9, -2.1):
        ...
        check(
            f"travel tracks engage orientation (yaw {engage_yaw:+.1f})",
    ...
    check(
        "position_scale scales translation only",
...
def test_clutch_translation_world_frame():
    ...
    for side in SIDES:
        for _ in range(24):
            ...
            flat_moved = (
                c.target(_pose(R_e, t_e + flat)).translation() - ee0.translation()
            )
            horizontal &= abs(float(flat_moved[2])) < 1e-12
            length &= (
                abs(float(np.linalg.norm(flat_moved) - np.linalg.norm(flat))) < 1e-12
            )
    ...
    check(
        "engage heading rotates the horizontal mapping (not odom-locked)",
    ...
    for side in SIDES:
        ...
        same &= close(
            pair[0].target(turn).rotation().as_matrix(),
            pair[1].target(turn).rotation().as_matrix(),
            1e-12,
        )
        # And engaging is still a zero-delta anchor in either frame
        same &= all(
            close(c.target(_pose(R_e, t_e)).translation(), ee0.translation(), 1e-12)
            for c in pair
        )
    check("rotation and the engage anchor are identical in both frames", same)

    check("position_scale still scales translation only", _world_scale_only())
...
def _world_scale_only() -> bool:
    ...
    return (
        close(half.target(moved).translation(), 0.5 * full.target(moved).translation())
        and abs(float(np.linalg.norm(half.target(moved).rotation().log())) - 0.5) < 1e-9
    )
...
def test_clutch_rotation():
    ...
    for side in SIDES:
        for axis in AXES:
            ...
            expected = (
                ee0.rotation().as_matrix()
                @ Rotation.from_rotvec(theta * YOR_WRIST_AXES[side][axis]).as_matrix()
            )
            check(
                f"{side} turn about {axis} turns the EE about {axis}",
...
def test_clutch_pinned_orientation():
    ...
    check(
        "wrist frame puts the push through the convention rotation",
        close(
            c_w.target(wrist).translation(),
            ee0.rotation().as_matrix() @ convention_matrix("left") @ [0.10, 0.0, 0.0],
        ),
    )
    ...
    seen = []
    for engage_rot, ee_rot in (
        (mink.SO3.identity(), home),
        (mink.SO3.from_z_radians(2.3), mink.SO3.from_y_radians(-0.8)),
    ):
    ...
    check(
        "pin_rotation survives re-engage",
    ...
    for side in SIDES:
        ...
        check(
            f"{side} operator_frame origin is the wrist target",
            close(
                c.operator_frame(wrist).translation(),
                c.wrist_target(wrist).translation(),
                1e-12,
            )
            and not close(
                c.operator_frame(wrist).rotation().as_matrix(),
...
def test_wrist_offset():
    ...
    for side in SIDES:
        ...
        mujoco.mj_forward(m, d)
        check(
            f"{side} offset is rigid across arm poses",
    ...
    ee0 = mink.SE3.from_rotation_and_translation(
        mink.SO3.from_x_radians(0.3), np.array([0.3, -0.25, 0.4])
    )
    ...
    check(
        "offset makes the wrist the pivot, not the flange",
        ...
        and np.linalg.norm(c.target(turned).translation() - ee0.translation()) > 0.01,
    )

    ee0 = mink.SE3.from_rotation_and_translation(
        mink.SO3.from_y_radians(0.9), np.array([0.3, -0.25, 0.4])
    )
...
def test_operator_frame_lands_on_the_ik_target():
    ...
    for side in SIDES:
        ...
        for frame in Clutch.TRANSLATION_FRAMES:
            ...
            at_engage &= close(
                c.operator_frame(_pose(R0, o0)).translation(), wrist_home
            )
            for rpy, origin in (
                ((0.5, 0.0, 0.8), [0.2, -0.3, 0.9]),
                ((-1.1, 0.7, 0.2), [0.3, 0.1, 1.0]),
            ):
                R = mink.SO3.from_rpy_radians(*rpy).as_matrix()
                got, want = (
                    c.operator_frame(_pose(R, origin)),
                    c.wrist_target(_pose(R, origin)),
                )
                tracks &= close(got.translation(), want.translation())
                tracks &= close(got.rotation().as_matrix(), want.rotation().as_matrix())
        check(f"{side} operator frame is the robot's wrist at engage", at_engage)
        check(f"{side} operator frame tracks the commanded wrist everywhere", tracks)
...
def test_wire_decode():
    print("\nWire decode")
    T_od = _pose(mink.SO3.from_rpy_radians(0.3, -0.2, 1.1).as_matrix(), [1.0, 2.0, 0.5])
    ...
    stream._ingest(
        {
            ...
            "home_seq": 0,
            "left": {
                "T_odom_hand": T_oh.astype(np.float32),
                "qpos": np.zeros(20, np.float32),
                "paused": False,
            },
        }
    )
    ...
    check(
        "T_odom_hand passes through as the wrist pose",
    ...
    stream = AriaHandStream("localhost", sides=("left",))
    stream._ingest(
        {"T_odom_device": T_od, "left": {"T_device_hand": T_dh, "paused": False}}
    )
    check(
        "a pre-wire-2 publisher is composed locally",
    ...
    stream = AriaHandStream("localhost", sides=("left",))
    stream._ingest(
        {"T_odom_device": T_od, "left": {"T_device_wrist": T_dh, "paused": False}}
    )
    check(
        "a hand-frame-less publisher leaves the side unfollowable",
...
def test_home_seq():
    ...
    check("joining a publisher mid-session adopts without firing", not w.update(5))
...
def test_staleness_gate():
    ...
    check(
        "fresh sample is not forced paused", stream.snapshot()["left"].paused is False
    )
    stream._t_recv -= 0.5
    check("stale sample is forced paused", stream.snapshot()["left"].paused is True)

    stream = AriaHandStream("localhost", sides=("left",), stale_s=None)
    stream._ingest({"left": {"T_odom_hand": T_oh, "paused": False}})
    stream._t_recv -= 10.0
    check("stale_s=None disables the gate", stream.snapshot()["left"].paused is False)
...
def test_aria_source():
    ...
    check(
        "AriaSource is an InputSource with the full contract",
        issubclass(AriaSource, InputSource)
        and all(callable(getattr(AriaSource, m)) for m in ("start", "stop", "update"))
    ...
    check(
        "source.py never touches the RPC client directly", "self.yor." not in src_text
    )

    ee = mink.SE3.from_rotation_and_translation(
        mink.SO3.from_x_radians(0.5), np.array([0.3, -0.25, 0.4])
    )
    other = mink.SE3.from_rotation_and_translation(
        mink.SO3.identity(), np.array([-0.3, -0.25, 0.4])
    )
    ...
    check(
        "nothing but the arm target is commanded",
        cmd.left_gripper is None
        and cmd.right_gripper is None
        and not cmd.home_left
        and not cmd.home_right
        and not cmd.home_arms
        and not cmd.home_lift
        and not cmd.toggle_fix_base
        and not cmd.toggle_collisions
        and not cmd.quit,
    )
    check("the idle hand is left alone", cmd.right_target is None)

    # The lift is the solver's, on every tick and not just after the first:
    # both nodes start with lift_target None, which they read as "the lift is
    # yours", and this backend never says otherwise. The first tick is the one
    # that used to pin it, so it is the one worth asserting on.
    check("the lift is not commanded on the first tick", cmd.lift_target is None)
    check(
        "the lift is not commanded on any later tick",
        src.update(state, 1 / 30).lift_target is None
        and src.update(state, 1 / 30).lift_target is None,
    )
    ...
    cmd = src.update(state, 1 / 30)
    check(
        "clutch reseed anchors on the server's actual EE",
        close(cmd.left_target.translation(), server_ee.translation())
        and close(
            cmd.left_target.rotation().as_matrix(), server_ee.rotation().as_matrix()
        ),
    )
    ...
    cmd = src.update(state, 1 / 30)
    check(
        "--no-clutch-reseed anchors on the local target",
    ...
    def run(src, ticks=10):
        return [
            c
            for c in (src.update(state, 1 / 30) for _ in range(ticks))
    ...
    fired = run(src)
    check(
        "a counter bump runs the node's home_arms sequence",
        len(fired) == 1
        and fired[0].home_arms
        and not fired[0].home_left
        and not fired[0].home_right,
    ...
    src.start()
    check("a counter that is merely nonzero at connect does not home", not run(src))
    ...
    cmd = src.update(state, 1 / 30)
    check(
        "a paused publisher releases the clutch",
...
def test_marker_rides_the_wrist():
    ...
    offset = {
        side: d.site(f"{side}_arm_ee").xmat.reshape(3, 3).T
        @ (d.body(f"{side}_wuji_hand_orient").xpos - d.site(f"{side}_arm_ee").xpos)
        for side in SIDES
    }
    ...
    for side in SIDES:
        ...
        fixed = np.linalg.norm(targets[side].translation() + R @ offset[side] - hand)
        ...
        check(
            f"{side} marker sits on the WUJI hand",
            fixed < 1e-3,
            f"{fixed * 1e3:.4f} mm",
        )
        check(
            f"{side} flange is a real distance behind it",
            flange > 0.03,
            f"{flange * 1e3:.1f} mm",
        )
...
def test_overlay_offset_ordering():
    ...
    check(
        "update_from_mjdata → sync_overlay_offset → draw_*", i_update < i_sync < i_draw
    )

    import re

    names = re.findall(
        r"(?:add_frame|add_line_segments|add_point_cloud|"
        r'remove_by_name)\(\s*(?:server,\s*)?f?"([^"]+)"',
        text,
    )
...
def test_config():
    ...
    check(
        "scene_path() resolves to an absolute path that exists",
        cfg.scene_path().is_absolute() and cfg.scene_path().exists(),
        str(cfg.scene_path()),
    )
    check(
        "hand is one the sources accept",
        cfg.mapping["hand"] in ("left", "right", "both"),
    )
    check(
        "translation_frame is one the clutch accepts",
    ...
    check(
        "a partial file keeps its override and defaults the rest",
        cfg.publisher["host"] == "10.0.0.9"
        and cfg.publisher["port"] == 5555
        and cfg.clutch["reseed"] is True,
    )
