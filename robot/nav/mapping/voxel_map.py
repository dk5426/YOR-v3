...
class GlobalVoxelMap:
    ...
    def __init__(
        ...
        self.max_insert_depth = 5.0  # matches carver depth range -2.0
        ...
        # ---- Per-voxel consecutive observation age (int16, GPU) ----
        # Incremented each frame the voxel is observed, reset to 0 when cleared.
        # Used for live→permanent promotion: voxels reaching promote_age are stable.
        # Set promote_age=0 to disable promotion (single-map mode).
        self._age: torch.Tensor = torch.zeros(0, dtype=torch.int16, device=self.device)
        self.promote_age: int = 0  # 0 = disabled; set to e.g. 15 for live map
    ...
    def integrate(
        ...
        if camera_pose is not None and self.max_insert_depth > 0:
            cam_t = torch.as_tensor(camera_pose[4:7], device=self.device, dtype=torch.float32)
            diff = pts - cam_t
    ...
    def _integrate_surface(
        ...
        if len(existing_src) > 0:
            eidx = torch.from_numpy(existing_idx.astype(np.int64)).to(self.device)
            esrc = existing_src.astype(np.int32)

            hits_np = hit_counts[esrc].astype(np.float32)
            hits_t = torch.from_numpy(hits_np).to(self.device)
            ...
            alpha_3 = torch.from_numpy(alpha_np).to(self.device).float().unsqueeze(1)

            hits_t_3d = hits_t.unsqueeze(1)

            old_color = self._color[eidx].float()
            incoming_mean_color = torch.from_numpy(color_sums[esrc]).to(self.device) / hits_t_3d
            new_color = old_color * (1 - alpha_3) + incoming_mean_color * alpha_3

            old_centroid = self._centroid[eidx].float()
            incoming_mean_centroid = torch.from_numpy(point_sums[esrc]).to(self._centroid.device) / hits_t_3d
        ...
        if len(new_src) > 0:
            ...
            self._keys[start:start + n_new] = torch.from_numpy(new_keys_np)

            hits_1d = hit_counts[nsrc].astype(np.float32)
            hits_t = torch.from_numpy(hits_1d).to(self.device)
            ...
            self._color[start:start + n_new]    = torch.from_numpy(color_sums[nsrc]).to(self._log_odds.device) / w_new_3d
            self._centroid[start:start + n_new] = torch.from_numpy(point_sums[nsrc]).to(self._log_odds.device) / w_new_3d
    ...
    def _clear_dynamic_objects(
        ...
        t_gpu = torch.as_tensor(t, device=self.device, dtype=torch.float32)
        ...
        rot_gpu = torch.from_numpy(rot).to(self.device, dtype=torch.float32)

        # 2. Build Depth Buffer entirely on GPU (no CPU→GPU transfer for pts)
        depth_buffer_gpu = torch.full((H, W), 20.0, device=self.device, dtype=torch.float32)
        observed_mask_gpu = torch.zeros((H, W), device=self.device, dtype=torch.bool)
        ...
        with torch.no_grad():
            try:
                keys_t = self._keys[:self._count].to(self.device) # (N, 3)
                ...
                if len(vc_t) > 0:
                    ...
                    #                  the image boundary. The outermost columns of a VIO
    ...
    def load(self, filename: str) -> None:
        ...
        with self._lock:
            self.vs = vs
            cap = max(n * 2, 50_000)
            self._keys = torch.zeros((cap, 3), dtype=torch.int32, device='cpu')
            self._log_odds = torch.zeros(cap, dtype=torch.float16, device='cpu')
            
            if color is not None:
                self._color = torch.zeros((cap, 3), dtype=torch.float32, device='cpu')
            else:
                self._color = None
            if centroid is not None:
                self._centroid = torch.zeros((cap, 3), dtype=torch.float32, device='cpu')
            else:
                self._centroid = None

            self._keys[:n] = torch.from_numpy(keys)

            if "log_odds" in data:
                self._log_odds[:n] = torch.from_numpy(data["log_odds"])
                if color_sum is not None:
                    self._color = torch.zeros((cap, 3), dtype=torch.float32, device='cpu')
                    self._color[:n] = torch.from_numpy(color_sum)  # assuming it was already averaged
                    
            elif "tsdf" in data:
                # Legacy format: convert TSDF to log_odds representation
                tsdf = torch.from_numpy(data["tsdf"])
                weight = torch.from_numpy(data["weight"])
                occ_mask = tsdf < 0.3
                ...
                if color_sum is not None:
                    self._color = torch.zeros((cap, 3), dtype=torch.float32, device='cpu')
                    self._color[:n] = torch.from_numpy(color_sum) / torch.clamp_min(weight.unsqueeze(1).float(), 1.0)
                    
            elif "hits" in data:
                # Ancient legacy format
                hits = torch.from_numpy(data["hits"])
                occ_mask = hits > 0
                self._log_odds[:n][occ_mask] = self.l_hit
                self._log_odds[:n][~occ_mask] = 0.0
                ...
                if color_sum is not None:
                    self._color = torch.zeros((cap, 3), dtype=torch.float32, device='cpu')
                    self._color[:n] = torch.from_numpy(color_sum) / torch.clamp_min(hits.unsqueeze(1).float(), 1.0)

            if color is not None:
                self._color[:n] = torch.from_numpy(color)
            if centroid is not None:
                self._centroid[:n] = torch.from_numpy(centroid)
