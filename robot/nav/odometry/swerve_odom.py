...
# ── Robot geometry ───────────────────────────────────────────────────────────
# These are ODOMETRY-CALIBRATED values, deliberately larger than the nominal
# LENGTH/WIDTH in robot/base_motor.py (0.1225 / 0.170), which are the CAD
# dimensions the inverse kinematics uses to *command* the modules. Fitting the
# forward model against measured motion absorbs steer-offset and tyre-scrub
# error into the effective module radius. Do not "correct" these to match
# base_motor.py — re-run the calibration instead.
...
class SwerveOdom:
    ...
    def _solve_fk_displacement(
        ...
        module_positions: np.ndarray = MODULE_POSITIONS,
    ) -> np.ndarray:
        """Solve for [dx, dy, dθ] from wheel steer angles and displacements.

        Args:
            steer_rad:           (N,) steer angle per module [rad]
            wheel_displacements: (N,) linear displacement per wheel [m]
            module_positions:    (N, 2) [rx, ry] for the modules

        Returns:
            (3,) array [dx_robot, dy_robot, dtheta] in robot frame,
            or np.zeros(3) if fewer than 3 wheels provided.
        """
        n = len(steer_rad)
        if n < 3:
            # Cannot solve 3-DOF system with fewer than 3 constraints
            return np.zeros(3)
    ...
    def update(self, steer_rad: np.ndarray, drive_counts: np.ndarray, dt: float) -> np.ndarray:
        ...
        disp = self._solve_fk_displacement(
            steer_rad,
            wheel_displacements,
        )
        ...
        # VIO Y-up: camera faces backward relative to the robot.
