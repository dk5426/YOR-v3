...
class AriaConfig:
    ...
    def scene_path(self) -> Path:
        """The MJCF scene for this session.

        `mapping.scene`, if set, always wins; otherwise auto-resolved from
        `hand.type` (robot/hand/hands.py::scene_for_hand_type). Call this
        *after* applying any CLI override to `hand.type` (e.g. `--hand`),
        not at construction time -- `hands_from_args()`/`resolved_aria_
        config()` mutate `cfg.hand["type"]` after `AriaConfig.load()`
        returns, so resolving eagerly in `__init__` would ignore it.
        """
        raw = self.mapping["scene"]
        if raw:
            p = Path(raw)
            return p if p.is_absolute() else _REPO / p
        from robot.hand.hands import scene_for_hand_type

        return scene_for_hand_type(self.hand["type"])
